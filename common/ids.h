...
struct ProjectionID {
  ...
  TableID table_id;
  ProjectionOffset projection_off;  // projection is native number means that
                                    // projection is a single column;
...
struct ColumnID {
  ...
  template <class Archive>
  void serialize(Archive& ar, const unsigned int version) {
    //		ar & projection_id & column_off & partitioner &
    // fileLocations &
...
struct PartitionID {
  ...
  std::string getName() const {
    ...
    if (projection_id.projection_off > 0) {
      ...
      str << "T" << projection_id.table_id << "C"
